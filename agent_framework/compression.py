...
class SelectiveCompressor:
    ...
    def estimate_token_reduction(
        ...
        return {
            ...
            'reduction_pct': round(tokens_saved / original_tokens * 100, 1) if original_tokens > 0 else 0
        }
...
class SemanticCompressor:
    ...
    def _get_compressor(self):
        ...
        if self._compressor_client is None:
            ...
            self._compressor_client = LLMClient(LLMConfig(
...
Output: Concise summary of relevant facts only."""
    ...
    async def batch_compress(
        ...
        tasks = [
            self.compress_text(content, query, target)
            for content, query, target in items
        ]
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        if self.total_original_tokens == 0:
            return {
                ...
                'message': 'No compressions logged yet'
            }
        ...
        avg_roi = (
            ...
            if self.total_compression_cost_usd > 0 else float('inf')
        )
...
class AdaptiveCompressor:
    ...
    async def compress(
        ...
        content: str,
        query: str,
        data_type: str = 'text'  # 'text' or 'fundamentals'
    ) -> str:
...
class CompressionQualityChecker:
    ...
    def get_quality_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'recommendation': (
                'Safe to use' if quality_maintained / len(self.checks) > 0.9
...
async def compress_if_needed(
    ...
    if should_compress(content, threshold):
        ...
        return await compressor.compress_text(
            content,
            query,
            target_tokens=threshold // 2
        )
    else:
        return content
...
def format_fundamentals_compressed(
    data: Dict[str, Any],
    focus: str = 'general',
    method: Literal['selective'] = 'selective'
) -> str:
...
def get_semantic_compressor(
    ...
    if _semantic_compressor is None:
        _semantic_compressor = SemanticCompressor(model, provider)
