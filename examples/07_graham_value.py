    ...
    async def analyze(self, ticker: str, data: dict) -> Signal:
        ...
        if score >= 3:
            ...
            return Signal(
                direction='bearish',
                confidence=0.6,
                reasoning=f'Value Score: {score} (bearish threshold: 0). ' + '; '.join(reasons) if reasons else 'No value criteria met'
            )
        else:
            return Signal(
                direction='neutral',
                confidence=0.5,
                reasoning=f'Value Score: {score} (between 0 and 3). ' + '; '.join(reasons) if reasons else 'Neutral score'
            )
...
async def main():
    """Example usage."""
    print(f"{'-' * 60}")
    print(f"GrahamValueAgent - Score-Based Strategy")
    print(f"{'-' * 60}\n")
