...
async def seed_news(db):
    ...
    news_data = {
        'AAPL': [
            {'date': date(2025, 10, 20), 'headline': 'Apple unveils new AI features in iOS 19', 'sentiment': 'positive'},
            {'date': date(2025, 10, 18), 'headline': 'iPhone sales exceed expectations in Q3', 'sentiment': 'positive'},
            {'date': date(2025, 10, 15), 'headline': 'Analysts raise AAPL price target to $200', 'sentiment': 'positive'},
        ],
        'MSFT': [
            {'date': date(2025, 10, 21), 'headline': 'Microsoft Azure revenue grows 30% YoY', 'sentiment': 'positive'},
            {'date': date(2025, 10, 19), 'headline': 'New enterprise AI tools drive adoption', 'sentiment': 'positive'},
            {'date': date(2025, 10, 16), 'headline': 'Microsoft announces stock buyback program', 'sentiment': 'positive'},
        ],
        'TSLA': [
            {'date': date(2025, 10, 22), 'headline': 'Tesla deliveries miss Q3 estimates', 'sentiment': 'negative'},
            {'date': date(2025, 10, 20), 'headline': 'New Gigafactory announced in Mexico', 'sentiment': 'positive'},
            {'date': date(2025, 10, 17), 'headline': 'FSD Beta shows promising improvements', 'sentiment': 'positive'},
        ],
        'JPM': [
            {'date': date(2025, 10, 21), 'headline': 'JPMorgan reports strong Q3 earnings', 'sentiment': 'positive'},
            {'date': date(2025, 10, 18), 'headline': 'Investment banking revenue up 15%', 'sentiment': 'positive'},
            {'date': date(2025, 10, 16), 'headline': 'Dividend increased by 10%', 'sentiment': 'positive'},
        ]
    }
        ...
        for ticker, news_items in news_data.items():
            for item in news_items:
                await conn.execute(
                    ...
                    VALUES ($1, $2, $3, $4, $5)
                    """,
                    ticker, item['date'], item['headline'], item['sentiment'], 'Example News'
                )
    ...
    async with db.acquire() as conn:
        for ticker, content in filings.items():
            await conn.execute(
                ...
                VALUES ($1, $2, $3, $4)
                """,
                ticker, '10-K', datetime.now().date(), content
            )
