...
class TestDataConfig:
    ...
    def test_mock_data_config(self):
        ...
        config = TestDataConfig(
            source="mock",
            data={"pe_ratio": 20.0, "roe": 15.0, "profit_margin": 12.0},
            ticker="AAPL",
        )
    ...
    def test_database_config(self):
        ...
        config = TestDataConfig(source="database", ticker="MSFT")
    ...
    def test_pdf_config(self):
        ...
        config = TestDataConfig(source="pdf", uploaded_file=mock_file, ticker="AAPL")
...
class TestDataPreparation:
    """Test data preparation logic (without importing non-existent pages)."""

    def test_mock_data_includes_company_name(self):
        """Test mock data preparation adds company name."""
        from gui.components.test_config import TestDataConfig

        config = TestDataConfig(source="mock", data={"pe_ratio": 20}, ticker="TEST")
        ...
        # When used, should add name (this is done in the page, not in TestDataConfig)
        # Just verify the config is correct
        assert config.data is not None

    def test_yfinance_data_structure(self):
        """Test YFinance data structure."""
        from gui.components.test_config import TestDataConfig

        fetched_data = {"pe_ratio": 25.0, "roe": 18.0, "name": "Apple Inc."}

        config = TestDataConfig(source="yfinance", data=fetched_data, ticker="AAPL")

        assert config.data == fetched_data
        assert config.data["name"] == "Apple Inc."

    def test_database_config_no_data(self):
        """Test database configuration has no immediate data."""
        from gui.components.test_config import TestDataConfig

        config = TestDataConfig(source="database", ticker="MSFT")

        # Database data fetched internally by executor, not in config
        assert config.data is None
        assert config.source == "database"

    def test_pdf_config_structure(self):
        """Test PDF configuration structure."""
        from gui.components.test_config import TestDataConfig

        config = TestDataConfig(source="pdf", uploaded_file=Mock(), ticker="AAPL")

        # RAG agents use PDF, not data dict
        assert config.source == "pdf"
        assert config.uploaded_file is not None
...
class TestYFinanceIntegration:
    ...
    def test_fetch_yfinance_data_missing_values(self):
        ...
        pe = mock_info.get("trailingPE") if mock_info.get("trailingPE") is not None else mock_info.get("forwardPE", 0)
        roe = (mock_info.get("returnOnEquity") or 0) * 100
